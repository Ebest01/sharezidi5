...
console.log(`[INIT] ShareZidi MongoDB Server - Using CORRECT EasyPanel Credentials`);
...
console.log('[MONGO] Using credentials:');
...
async function connectToMongo() {
  try {
    ...
    // Use 'sharezidi' as database name (can be anything since EasyPanel doesn't specify)
...
app.get('/api/dbtest', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      credentials: 'EasyPanel-szmdb_user',
      version: 'Fixed'
    });
...
// User registration endpoint - Using your auto-password format
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    if (dbConnected) {
      ...
      console.log(`❌ [SERVER] Database connection failed - check credentials`);
    }
  });
}
