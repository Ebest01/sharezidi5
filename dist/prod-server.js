...
var GeolocationService = class {
   ...
   */
  static getFallbackLocationData(ip, reason) {
    return {
      ip,
      country: "Unknown",
      country_code: "XX",
   ...
   */
  static normalizeLocationData(data, ip) {
    return {
      ip,
...
var MONGODB_URI = process.env.MONGODB_URI || "mongodb://shzmdb2:11xxshzMDB@193.203.165.217:27017/sharezidi?authSource=admin&ssl=false";
...
var Visitor = mongoose2.model("Visitor", visitorSchema);
...
var insertVisitorSchema = z.object({
...
app.get("/api/analytics", async (req, res) => {
  try {
    ...
    res.json({
      ...
      countryBreakdown: Object.entries(countryStats).sort(([, a], [, b]) => b - a).slice(0, 10).map(([country, count]) => ({ country, count })),
      recentVisitors: totalVisitors.slice(-10).map((v) => ({
        city: v.city,
        country: v.country,
...
var publicPath = path.join(process.cwd(), "dist", "public");
